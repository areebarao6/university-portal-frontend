...
export default function Fees() {
  ...
  const handleDownloadVoucher = () => {
    ...
    const feeRows = [["Total Fee", totalFee], ["Amount Paid", paidFee], ["Pending Amount", pendingFee]];
    feeRows.forEach(([label, amount], i) => {
      const y = 155 + i * 11;
      if (i % 2 === 0) { doc.setFillColor(248, 250, 252); doc.rect(14, y - 7, pageWidth - 28, 11, "F"); }
      doc.setTextColor(75, 85, 99);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(10);
      doc.text(label, 22, y);
  ...
  return (
    <div className="flex flex-col md:flex-row bg-gray-100 min-h-screen overflow-x-hidden">
      ...
      <div className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto no-scrollbar flex flex-col items-center pt-16 md:pt-8">
        ...
        {/* Skeleton while loading, real card when ready */}
        {loading ? (
          <div className="w-full max-w-md sm:max-w-lg lg:max-w-xl rounded-2xl overflow-hidden">
            <div className="animate-pulse bg-indigo-300 rounded-2xl p-6 sm:p-8 space-y-4">
              <div className="h-4 bg-indigo-200 rounded w-1/3" />
              <div className="h-6 bg-indigo-200 rounded w-2/3" />
